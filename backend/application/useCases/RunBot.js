...
class RunBot {
     ...
     */
    constructor({ botRepository, commandRunner, messagePort, fileRepository, logger }) {
    ...
    async execute(botName) {
        ...
        if (!bot || !bot.botPath || !this.fileRepository.exists(bot.botPath)) {
            ...
            this.messagePort.sendError(botName, errMsg);
            return;
        }
        ...
        try {
            ...
            this.messagePort.sendProgress(botName, "Iniciando...", "Arrancando bot de Python...", 10);

            // We pass the messagePort to the runner so it can stream stdout
            await this.commandRunner.runPythonScript(options, fileName, botName, this.messagePort);

            log.success('Ejecución finalizada correctamente.');
            this.messagePort.sendSuccess(botName, "Ejecución finalizada.");
        } catch (e) {
            ...
            if (session) session.close();
            this.messagePort.sendError(botName, `La automatización se ejecutó con un error severo: ${e.message}`);
        }
    }
}
